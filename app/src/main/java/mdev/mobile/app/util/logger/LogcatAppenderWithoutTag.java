...
//TODO: For now, we initialize logback via runtime. Do we we still need @Keep here?
@Keep
public class LogcatAppenderWithoutTag extends UnsynchronizedAppenderBase<ILoggingEvent> {
    /**
     ...
     */
    public LogcatAppenderWithoutTag() {
    }
        ...
        switch (event.getLevel().levelInt) {
            ...
            case Level.TRACE_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.VERBOSE)) {
                    Log.v("T", this.encoder.getLayout().doLayout(event));
                }
                break;

            case Level.DEBUG_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.DEBUG)) {
                    Log.d("D", this.encoder.getLayout().doLayout(event));
                }
                break;

            case Level.INFO_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.INFO)) {
                    Log.i("I", this.encoder.getLayout().doLayout(event));
                }
                break;

            case Level.WARN_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.WARN)) {
                    Log.w("W", this.encoder.getLayout().doLayout(event));
                }
                break;

            case Level.ERROR_INT:
                if (!checkLoggable || Log.isLoggable(tag, Log.ERROR)) {
                    Log.e("E", this.encoder.getLayout().doLayout(event));
                }
                break;
